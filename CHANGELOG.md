...
### Fixed
- `grove setup` no longer strips observability services from `docker-compose.yml` when all stack options are enabled. The compose parser incorrectly treated nested `volumes:` keys inside services as the top-level volumes section, removing every service after `db`.
...
[2.3.1]: https://github.com/caiolandgraf/grove/releases/tag/v2.3.1
